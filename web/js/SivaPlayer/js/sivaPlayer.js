...
function SivaPlayer(DOMObject, arrayPosition){
			...
			if(e.keyCode == '27'){
				...
				if($(e.target).prop('tagName') == 'INPUT' || $(e.target).prop('tagName') == 'TEXTAREA'){
					...
					if(videoElement.length > 0){
						thisPlayer.logAction('useKey', 'SPACE', ((thisPlayer.paused()) ? 'play' : 'pause'));
	...
	this.setScene = function(scene, startTime, autostart){
		...
		if($('.sivaPlayer_mainVideo', this.player).length == 0 || this.configuration.scenes[scene].type == 'youtube' || this.isMobile){
			$('.sivaPlayer_videoBackground', this.player).remove();
			var videoElement;
			if(this.configuration.scenes[scene].type != 'youtube'){
				videoElement = '<video class="sivaPlayer_mainVideo">';
				...
				for(var i = 0; i < this.configuration.scenes[scene].files.length; i++){
					...
					if(testElement.canPlayType(file.type) != ''){
						...
						videoElement += '<source src="' + this.appendAccessToken(this.getFullURL(file.url[this.currentLanguage].href + "." + file.format)) + '" type="' + file.type + '" />';
					}
				}
				...
				videoElement = '<iframe class="sivaPlayer_mainVideo" id="youtube" type="text/html" src="' + this.configuration.scenes[scene].files[0].url[this.currentLanguage].href + '?controls=0&disablekb=1&enablejsapi=1&modestbranding=1&playerapiid=siva&showinfo=0&iv_load_policy=3&rel=0' + ((this.configuration.scenes[scene].start) ? '&start=' + this.configuration.scenes[scene].start : '') + '' + ((this.configuration.scenes[scene].end) ? '&end=' + this.configuration.scenes[scene].end : '') + '" frameborder="0" /><div class="sivaPlayer_mainVideoHelper"></div>';
			}
			...
			$('.sivaPlayer_videoBackground', thisPlayer.player).click(function(){
				...
				if(thisPlayer.mainVideoExists()){
					prevPaused = thisPlayer.paused();
				}
				if(thisPlayer.mainVideoExists() && prevPaused == thisPlayer.paused()){
					if(!thisPlayer.paused()){
			...
			for(var i = 0; i < this.configuration.scenes[scene].files.length; i++){
				...
				if($.inArray(file.type, this.configuration.supportedVideoTypes) > -1){
					$('.sivaPlayer_mainVideo', this.player).attr('src', this.appendAccessToken(this.getFullURL(file.url[this.currentLanguage].href + "." + file.format)));
					break;
				}
			}
		}
		...
		this.getCommunityAnnotations(scene, function(){
			if(thisPlayer.configuration.scenes[scene].type != 'youtube'){
				...
				$.getScript('https://www.youtube.com/iframe_api', function(){
					thisPlayer.youtubeObject = new YT.Player('youtube', {
						events: {
							'onReady': function(){
								thisPlayer.configuration.ratio = this.videoWidth / this.videoHeight;
								thisPlayer.onLoadedData(scene, startTime, autostart);
							...
							'onStateChange': function(e){
								if(e.data == 0){
									thisPlayer.onSceneEnd(thisPlayer.youtubeObject);
								}
								else if(e.data == 1){
									$(thisPlayer.player).addClass('sivaPlayer_playing');
									thisPlayer.tidyPlayer();
	...
	this.onSceneEnd = function(videoElement){
		if(this.currentSceneEnded)
			return;
		if(this.youtubeObject == undefined){
			videoElement.pause();
		}
		$(this.player).addClass('sivaPlayer_replay');
		this.currentSceneEnded = true;
		if(this.currentSceneTime != this.duration()){
			if(this.youtubeObject == undefined){
				this.setCurrentSceneTime(this.duration());	
			}
			else{
				this.currentSceneTime = this.duration();
			}
	...
	this.pauseVideo = function(){
		...
		if(this.youtubeObject == undefined){
			...
			this.youtubeObject.pauseVideo();
		}
	};
	
	this.mainVideoExists = function(){
		var videoElement = $('.sivaPlayer_mainVideo', this.player);
		return (videoElement.length > 0 || this.youtubeObject != undefined);		
	};
	...
	this.paused = function(){
		if(this.youtubeObject == undefined){
			...
			return (this.youtubeObject.getPlayerState() == -1 || this.youtubeObject.getPlayerState() == 0 || this.youtubeObject.getPlayerState() == 2);
		}
	};
	...
	this.duration = function(){
		if(this.youtubeObject == undefined){
			...
			var duration = this.youtubeObject.getDuration();
			if(this.configuration.scenes[this.currentScene].end && this.configuration.scenes[this.currentScene].end < duration){
				duration = this.configuration.scenes[this.currentScene].end;
			}
			return duration - ((this.configuration.scenes[this.currentScene].start) ? this.configuration.scenes[this.currentScene].start : 0);
		}
	};
	...
	this.currentTime = function(){
		if(this.youtubeObject == undefined){
			...
			return this.youtubeObject.getCurrentTime() - ((this.configuration.scenes[this.currentScene].start) ? this.configuration.scenes[this.currentScene].start : 0);
		}
	};
	
	this.setPlayedBefore = function(exceptVideo){
		var thisPlayer = this;
		$.each($('video, audio, iframe.sivaPlayer_mainVideo', this.player), function(){
			if((($(this).hasClass('sivaPlayer_mainVideo') && !thisPlayer.paused()) || (!$(this).hasClass('sivaPlayer_mainVideo') && !this.paused)) && this != exceptVideo){
	...
	this.setVolume = function(isSetByUser, hideVolumeControl){
		...
		if(this.youtubeObject != undefined){
			this.youtubeObject.setVolume((thisPlayer.chromecastSession) ? 0 : thisPlayer.volume * 100);
		}
		if(isSetByUser){
			this.previousVolume = -1;
	...
	this.createControls = function(amountSubtitles, amountDisableableAnnotations){
		...
		var duration = this.formatTime(thisPlayer.duration());
	...
	this.createTopControls = function(node, isScene){
		...
		.click(function(){
			if((thisPlayer.history.length > 1 || thisPlayer.currentSceneTime > 5) && !$(this).hasClass('sivaPlayer_disabled')){
				...
				if(thisPlayer.mainVideoExists() && thisPlayer.currentTime() > 5){
		...
		.click(function(){
			if(isScene && !$(this).hasClass('sivaPlayer_disabled')){	
				...
				thisPlayer.playVideo();
				thisPlayer.sendChromecastMessage('setCurrentSceneTime', [thisPlayer.duration() - 0.5]);
				thisPlayer.setCurrentSceneTime(thisPlayer.duration() - 0.5);
	...
	this.createAnnotations = function(){
		...
		$('.sivaPlayer_richtextImage', this.player).attr('src', function(index, src){
			$(this).attr('src', thisPlayer.getFullURL(src));
		});
	...
	this.createAnnotation = function(annotation, triggerId, tabs){
		...
		if(annotation.type == 'image'){
			content = '<img src="' + this.appendAccessToken(((annotation.preview) ? this.getThumbnailURL(this.getFullURL(annotation.content[this.currentLanguage].href)) : this.getFullURL(annotation.content[this.currentLanguage].href))) + '" alt="' + this.getLabel('imageAnnotationAlt') + '" title="' + this.getLabel('imageAnnotationTooltip') + '" />';
			...
			for(; i < annotation.images.length; i++){
				...
				content += '<td><img src="' + this.appendAccessToken(this.getThumbnailURL(this.getFullURL(annotation.images[i][this.currentLanguage].href))) + '" alt="' + this.getLabel('imageAnnotationAlt') + '" title="' + this.getLabel('imageAnnotationTooltip') + '" class="sivaPlayer_galleryImage" /></td>';
			}
			...
			if(annotation.thumbnail && annotation.thumbnail[this.currentLanguage]){
				content += '<img src="' + this.appendAccessToken(this.getFullURL(annotation.thumbnail[this.currentLanguage].href)) + '" />';
			}
			else{
	...
	this.updateMediaAnnotationTimelines = function(alwaysUpdate){
		var thisPlayer = this;
		var element = $('.sivaPlayer_mediaPopup video, .sivaPlayer_mediaPopup audio', this.player)[0];
		if(!element){
			return;
		}
		...
		if(duration && !isNaN(duration)){
			...
			$('.sivaPlayer_mediaPopup .sivaPlayer_timelineProgress .sivaPlayer_timelineProgressBar span', this.player).css('width', percentage + '%');
			$('.sivaPlayer_mediaPopup .sivaPlayer_timelineCurrentTime', this.player).text(this.formatTime(currentTime));
		}
		if(!alwaysUpdate && !element.paused){
			setTimeout(function(){
				thisPlayer.updateMediaAnnotationTimelines(false);
			}, 500);
		}
	...
	this.createMediaZoom = function(annotationId, autostarted){
		...
		if(annotation.type == 'video'){
			...
			for(var i = 0; i < annotation.files.length; i++){
				content += '<source src="' + this.appendAccessToken(this.getFullURL(annotation.files[i].url[this.currentLanguage].href + '.' + annotation.files[i].format)) + '" type="' + annotation.files[i].type + '" />';
			}
			content += '</video>';
		}
		else{
	...
	this.createPdfZoom = function(annotationId){
		...
		$('.sivaPlayer_pdfPopup .sivaPlayer_content', this.player).empty().append(this.getLabel('pdfAnnotation_error') + '<iframe src="' + this.appendAccessToken(this.getFullURL(annotation.content[this.currentLanguage].href)) + '"></iframe>');
	};
	...
	this.createImageZoom = function(annotationId, index){
		...
		$('.sivaPlayer_zoomPopup .sivaPlayer_content', this.player).empty().append('<img src="' + this.appendAccessToken(this.getFullURL(((annotation.type == 'gallery') ? annotation.images[index][this.currentLanguage].href : annotation.content[this.currentLanguage].href))) + '" alt="' + this.getLabel('zoomImageTooltip') + '" />');
		if(annotation.type == 'gallery'){
			var thumbnails = '';
			for(var i = 0; i < annotation.images.length; i++){
				thumbnails += '<img src="' + this.appendAccessToken(this.getThumbnailURL(this.getFullURL(annotation.images[i][this.currentLanguage].href))) + '" />';
			}
	...
	this.getFullURL = function(url){
		if(url.substring(0, 1) == '/'){
			url = this.configuration.videoPath + url;
		}
		return url;
	};
	...
	this.setCurrentSceneTime = function(time, noVideoUpdate){
		var videoElements = $('.sivaPlayer_mainVideo', this.player);
		if(videoElements.length > 0 || this.youtubeObject != undefined){
			if(this.youtubeObject == undefined && !this.currentSceneEnded && time > this.duration()){
				...
				if(!noVideoUpdate){
					if(this.youtubeObject == undefined){
						...
						var start = ((this.configuration.scenes[this.currentScene].start) ? this.configuration.scenes[this.currentScene].start : 0);
						this.youtubeObject.seekTo((time + start), true);
					}
				}
			}
	...
	this.updateTimeline = function(alwaysUpdate){
		var thisPlayer = this;
		if((this.mainVideoExists() && !this.paused()) || alwaysUpdate){
			if(!alwaysUpdate){
				this.setCurrentSceneTime(this.currentTime(), true);
			}
			var percentage = (parseInt((this.currentSceneTime * 100 / this.duration()) * 100) / 100);
			if(percentage > 100){
				percentage = 100;
			}
